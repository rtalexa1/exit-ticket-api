class ReflectionQuestionsController < ApplicationController
  ...
  # POST /reflection_questions
  def create
    @reflection_question = ReflectionQuestion.new(reflection_question_params)

    if @reflection_question.save
      render json: @reflection_question, status: :created, location: @reflection_question
    else
      render json: @reflection_question.errors, status: :unprocessable_entity
    end
  end
